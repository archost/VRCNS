...
namespace VRKeys {
	...
	public class Controller : MonoBehaviour {
		...
		private InputDeviceCharacteristics _role = InputDeviceCharacteristics.None;
		...
		private void Start () {
			...
			_role = (mallet.hand == Mallet.MalletHand.Left) ? InputDeviceCharacteristics.Left : InputDeviceCharacteristics.Right;
		}

		private InputDevice GetDevice () {
			if (_device.isValid) return _device;
			//if (role == InputDeviceRole.Unknown) return _device;
			if (_role == InputDeviceCharacteristics.None) return _device;
			
			List<InputDevice> devices = new List<InputDevice> ();
			//InputDevices.GetDevicesWithRole (role, devices);
			InputDevices.GetDevicesWithCharacteristics(_role, devices);
